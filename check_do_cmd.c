...
void ft_check_do_cmd(char **cmd_input, t_env *env)
{
    char      **envp;
    char	    *cmd_path;
    //compares your first word of input with the cmds, if it matches it resolves
    if(ft_strncmp(cmd_input[0], "echo", (ft_strlen(cmd_input[0]) + 1)) == 0)
       ...
       ft_cd(cmd_input, env);
         ...
         ft_pwd();
    else if(ft_strncmp(cmd_input[0], "unset", (ft_strlen(cmd_input[0]) + 1)) == 0)
        ft_unset(env, cmd_input[1]);
    else if(ft_strncmp(cmd_input[0], "export", (ft_strlen(cmd_input[0]) + 1)) == 0)
